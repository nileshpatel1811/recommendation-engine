function getRecommendations(answers) {
    ...
    const scoredLenders = lenders.map(lender => {
        ...
        if (answers.priority === "Best overall balance") {
            ...
            if (profile.trust === "very_high") {
                score += 4;
            } else if (profile.trust === "high") {
                score += 3;
            }
        ...
        if (answers.urgency === "Within a few days") {
            if (profile.speed === "fast") {
                score += 3;
            } else if (profile.speed === "moderate") {
                score += 2;
        ...
        if (answers.amount === "₹2 lakh - ₹5 lakh") {
            if (profile.loanAmount === "very_high") {
                score += 3;
            } else if (profile.loanAmount === "high") {
                score += 2;
    ...
    if (answers.purpose === "Switch an existing gold loan" && currentLender) {
...
function tieBreaker(a, b, priority, loanStyle) {
    ...
    if (priority === "Best overall balance") {
        const trustRank = {
            "very_high": 3,
            "high": 2,
            "normal": 1
        };
        return (trustRank[profileB.trust] || 0) - (trustRank[profileA.trust] || 0);
    }
